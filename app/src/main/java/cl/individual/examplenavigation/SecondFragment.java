...
import cl.individual.examplenavigation.databinding.FragmentSecondBinding;
...
public class SecondFragment extends Fragment {
    ...
    private void initListeners() {
        binding.txtSecondFrag.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                int opcionId = binding.trivia1.getCheckedRadioButtonId();
                int opcionCorrecta = binding.opcion3Wsp.getId();


                if (opcionId == opcionCorrecta) {
                    Navigation.findNavController(binding.getRoot()).navigate(R.id.action_secondFragment_to_fourthFragment);
                } else if (opcionId == -1) {
